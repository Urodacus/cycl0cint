cycl0cint Core version 0.16.1 is now available from:
...
installer (on Windows) or just copy over `/Applications/cycl0cint-Qt` (on Mac)
...
cycl0cint Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

cycl0cint Core should also work on most other Unix-like systems but is not
